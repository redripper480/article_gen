document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            const response = await fetch(GEMINI_API_URL, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `As an SEO expert, generate 15 unique and compelling titles based on the keyword "${keywords}".
    ...
    async function handleWriteArticle() {
        ...
        if (articleProgress && writeArticleBtn) {
            ...
            const outlineLines = selectedOutline.split('\n')
                .map(line => line.replace(/^\d+\.\s*/, '').trim())
                .filter(line => line && line.toLowerCase() !== 'introduction');

            const hasFAQs = outlineLines.some(line => line.toLowerCase() === 'faqs' || line.toLowerCase() === 'faq’s' || line.toLowerCase() === 'frequently asked questions');
            ...
            try {
                ...
                const articleText = data.choices[0].message.content.trim();
    ...
    async function generateOutline2(keywords, title) {
        ...
        try {
            ...
            const outlineText = data.choices[0].message.content.trim();
            const outline = outlineText.split('\n').map(line => line.trim());
            return outline.map((line, index) => `${index + 1}. ${line}`).slice(0, 13);
        } catch (error) {
